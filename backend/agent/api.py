...
def initialize(_thread_manager: ThreadManager, _db: DBConnection, _instance_id: str = None):
...
async def start_agent(
    ...
    await verify_thread_access(client, thread_id, user_id)
    thread_result = (
        await client.table("threads").select("project_id", "account_id").eq("thread_id", thread_id).execute()
    )
...
async def get_agent_runs(thread_id: str, user_id: str = Depends(get_current_user_id_from_jwt)):
    ...
    await verify_thread_access(client, thread_id, user_id)
    agent_runs = (
        await client.table("agent_runs").select("*").eq("thread_id", thread_id).order("created_at", desc=True).execute()
    )
...
async def stream_agent_run(agent_run_id: str, token: Optional[str] = None, request: Request = None):
    ...
    async def stream_generator():
        ...
        try:
            ...
            run_status = (
                await client.table("agent_runs").select("status").eq("id", agent_run_id).maybe_single().execute()
            )
                        ...
                        try:
                            ...
                            if task in tasks:
                                ...
                                if message and isinstance(message, dict) and message.get("channel") == response_channel:
                                    ...
                                    message and isinstance(message, dict) and message.get("channel") == control_channel
                                ):
