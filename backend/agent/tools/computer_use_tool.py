...
class ComputerUseTool(SandboxToolsBase):
    ...
    async def move_to(self, x: float, y: float) -> ToolResult:
        ...
        try:
            ...
            result = await self._api_request("POST", "/automation/mouse/move", {"x": x_int, "y": y_int})
    ...
    async def press(self, key: str) -> ToolResult:
        ...
        try:
            ...
            result = await self._api_request("POST", "/automation/keyboard/press", {"keys": key, "presses": 1})
    ...
    @xml_schema(
        tag_name="wait",
        mappings=[{"param_name": "duration", "node_type": "attribute", "path": "duration"}],
        example="""
        <wait duration="1.5">
        </wait>
        """,
