...
async def get_stripe_customer_id(client, user_id: str) -> Optional[str]:
    ...
    result = await client.schema("basejump").from_("billing_customers").select("id").eq("account_id", user_id).execute()
        ...
        if existing_subscription:
            ...
            try:
                ...
                if is_upgrade:
                    ...
                    await client.schema("basejump").from_("billing_customers").update({"active": True}).eq(
                        "id", customer_id
                    ).execute()
                    ...
                    return {
                        ...
                        "details": {
                            ...
                            "current_price": (
                                round(current_price["unit_amount"] / 100, 2) if current_price.get("unit_amount") else 0
                            ),
                            "new_price": (
                                round(new_price["unit_amount"] / 100, 2) if new_price.get("unit_amount") else 0
                            ),
                            "invoice": (
                                {
                                    ...
                                    "amount_due": (
                                        round(latest_invoice["amount_due"] / 100, 2) if latest_invoice else 0
                                    ),
                                    "amount_paid": (
                                        round(latest_invoice["amount_paid"] / 100, 2) if latest_invoice else 0
                                    ),
                                }
                                if latest_invoice
                                else None
            ...
            try:
                ...
                if next_phase:
                    ...
                    status_response.scheduled_plan_name = (
                        scheduled_tier_info.get("name", "unknown") if scheduled_tier_info else "unknown"
                    )
        ...
        if event.type in [
            ...
            if event.type == "customer.subscription.created" or event.type == "customer.subscription.updated":
                ...
                if subscription.get("status") in ["active", "trialing"]:
                    ...
                    has_active = (
                        len(stripe.Subscription.list(customer=customer_id, status="active", limit=1).get("data", []))
                        > 0
                    )

                    if not has_active:
                        await client.schema("basejump").from_("billing_customers").update({"active": False}).eq(
                            "id", customer_id
                        ).execute()
                        logger.info(
                ...
                has_active = (
                    len(stripe.Subscription.list(customer=customer_id, status="active", limit=1).get("data", [])) > 0
                )
