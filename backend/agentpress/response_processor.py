...
from typing import List, Dict, Any, Optional, Tuple, AsyncGenerator, Callable, Union, Literal, Set
...
class StreamingState:
    ...
    yielded_tool_indices: Set = field(default_factory=set)
    tool_index: int = 0
...
class ResponseProcessor:
    ...
    async def _process_chunk_and_create_messages(
        ...
        if (
            ...
            streaming_state.finish_reason = chunk.choices[0].finish_reason
            logger.debug(f"Detected finish_reason: {streaming_state.finish_reason}")
        ...
        if hasattr(chunk, "choices") and chunk.choices:
            ...
            if delta and hasattr(delta, "content") and delta.content:
                ...
                streaming_state.accumulated_content += chunk_content
                streaming_state.current_xml_content += chunk_content

                if not (config.max_tool_calls > 0 and streaming_state.tool_call_count >= config.max_tool_calls):
                ...
                if config.xml_tool_calling and not (
                    config.max_tool_calls > 0 and streaming_state.tool_call_count >= config.max_tool_calls
                ):
                    xml_chunks = self._extract_xml_chunks(streaming_state.current_xml_content)
                    for xml_chunk in xml_chunks:
                        streaming_state.current_xml_content = streaming_state.current_xml_content.replace(
                            xml_chunk, "", 1
                        )
                        streaming_state.xml_chunks_buffer.append(xml_chunk)
                        result = self._parse_xml_tool_call(xml_chunk)
                        if result:
                            tool_call, parsing_details = result
                            streaming_state.xml_tool_call_count += 1
                            streaming_state.tool_call_count += 1
                            current_assistant_id = (
                                streaming_state.last_assistant_message_object["message_id"]
                            ...
                            if config.execute_tools and config.execute_on_stream:
                                ...
                                streaming_state.pending_tool_executions.append(
                                    {
                                        "task": execution_task,
                                        "tool_call": tool_call,
                                        "tool_index": streaming_state.tool_index,
                                        "context": context,
                                    }
                                )
                                streaming_state.tool_index += 1

                            if config.max_tool_calls > 0 and streaming_state.tool_call_count >= config.max_tool_calls:
                                logger.debug(f"Reached tool call limit ({config.max_tool_calls})")
                                streaming_state.finish_reason = "tool_limit_reached"
                                break  # Stop processing more XML chunks in this delta

            # --- Process Native Tool Call Chunks ---
            if (
                config.native_tool_calling
                and not (config.max_tool_calls > 0 and streaming_state.tool_call_count >= config.max_tool_calls)
                and delta
                ...
                for tool_call_chunk in delta.tool_calls:
                    ...
                    if hasattr(tool_call_chunk, "model_dump"):
                        ...
                        if hasattr(tool_call_chunk, "function"):
                            ...
                            if hasattr(tool_call_chunk.function, "arguments"):
                                tool_call_data_chunk["function"]["arguments"] = tool_call_chunk.function.arguments
                    ...
                    if idx not in streaming_state.tool_calls_buffer:
                        streaming_state.tool_calls_buffer[idx] = {
                            "id": None,
                    ...
                    if hasattr(tool_call_chunk, "id") and tool_call_chunk.id:
                        streaming_state.tool_calls_buffer[idx]["id"] = tool_call_chunk.id

                    if hasattr(tool_call_chunk, "function"):
                        if hasattr(tool_call_chunk.function, "name") and tool_call_chunk.function.name:
                            streaming_state.tool_calls_buffer[idx]["function"]["name"] = tool_call_chunk.function.name

                        if hasattr(tool_call_chunk.function, "arguments") and tool_call_chunk.function.arguments:
                            current_args = streaming_state.tool_calls_buffer[idx]["function"]["arguments"]
                            streaming_state.tool_calls_buffer[idx]["function"]["arguments"] = (
                                current_args + tool_call_chunk.function.arguments
                            )

                    # Check if the tool call is complete
                    has_complete_tool_call = (
                        streaming_state.tool_calls_buffer[idx]["id"]
                        and streaming_state.tool_calls_buffer[idx]["function"]["name"]
                        and streaming_state.tool_calls_buffer[idx]["function"]["arguments"]
                    )

                    if has_complete_tool_call:
                        # Validate that arguments are valid JSON
                        try:
                            json.loads(streaming_state.tool_calls_buffer[idx]["function"]["arguments"])
                        except json.JSONDecodeError:
                            has_complete_tool_call = False

                    if has_complete_tool_call and config.execute_tools and config.execute_on_stream:
                        streaming_state.native_tool_call_count += 1
                        streaming_state.tool_call_count += 1
                        ...
                        current_tool = streaming_state.tool_calls_buffer[idx]
                        tool_call_data = {
                        ...
                        context = self._create_tool_context(
                            tool_call_data, streaming_state.tool_index, current_assistant_id
                        )
    ...
    async def process_streaming_response(
        ...
        try:
            async for msg in self._create_initial_status_messages(thread_id=thread_id, thread_run_id=thread_run_id):
                yield msg

            async for chunk in llm_response:
                async for msg in self._process_chunk_and_create_messages(
                    chunk=chunk,
                ...
                if streaming_state.finish_reason == "tool_limit_reached":
                    logger.info("Stopping stream processing after loop due to tool call limit")
                    break
            ...
            if streaming_state.finish_reason == "tool_limit_reached":
                ...
                logger.info(
                    ...
                    f" ({streaming_state.xml_tool_call_count} XML, {streaming_state.native_tool_call_count} native)"
                )
            ...
            if streaming_state.accumulated_content:
                ...
                if (
                    ...
                    and streaming_state.tool_call_count >= config.max_tool_calls
                    and streaming_state.xml_chunks_buffer
                ):
                    last_xml_chunk = streaming_state.xml_chunks_buffer[-1]
                    last_chunk_end_pos = streaming_state.accumulated_content.find(last_xml_chunk) + len(last_xml_chunk)
                    if last_chunk_end_pos > 0:
                        streaming_state.accumulated_content = streaming_state.accumulated_content[:last_chunk_end_pos]

                # ... (Extract complete_native_tool_calls logic) ...
                if config.native_tool_calling:
                    for idx, tc_buf in streaming_state.tool_calls_buffer.items():
                        # Only add tool calls up to the limit if a limit is set
                        if (
                            config.max_tool_calls > 0
                            and len(complete_native_tool_calls) + streaming_state.xml_tool_call_count
                ...
                if streaming_state.last_assistant_message_object:
                    ...
                    yield_metadata = json.loads(streaming_state.last_assistant_message_object.get("metadata", "{}"))
                    yield_metadata["stream_status"] = "complete"
                    yield {
                        **streaming_state.last_assistant_message_object,
            ...
            if config.execute_tools:
                ...
                if config.xml_tool_calling:
                    ...
                    xml_chunks = self._extract_xml_chunks(streaming_state.current_xml_content)
                    streaming_state.xml_chunks_buffer.extend(xml_chunks)
                    # Process only chunks not already handled in the stream loop
                    remaining_limit = (
                        config.max_tool_calls - streaming_state.tool_call_count
                        if config.max_tool_calls > 0
                        else len(streaming_state.xml_chunks_buffer)
                    )
                    xml_chunks_to_process = streaming_state.xml_chunks_buffer[
                        :remaining_limit
                    ...
                    for chunk in xml_chunks_to_process:
                        ...
                        if parsed_result:
                            ...
                            if not any(
                                exec["tool_call"] == tool_call for exec in streaming_state.pending_tool_executions
                            ):
                ...
                if tool_results_map:
                    ...
                    for tool_idx in sorted(tool_results_map.keys()):
                        ...
                        context.result = result
                        if not context.assistant_message_id and streaming_state.last_assistant_message_object:
                            context.assistant_message_id = streaming_state.last_assistant_message_object["message_id"]
            ...
            if streaming_state.last_assistant_message_object:  # Only calculate if assistant message was saved
                try:
                    ...
                    final_cost = completion_cost(
                        ...
                        completion=streaming_state.accumulated_content,
                    )
    ...
    def _find_next_tool_tag(self, content: str, pos: int) -> Tuple[int, Optional[str]]:
        """Find the next tool tag starting from pos."""
        next_tag_start = -1
        current_tag = None
        ...
        return next_tag_start, current_tag

    def _find_matching_end_tag(self, content: str, tag_name: str, start_pos: int) -> Tuple[int, int]:
        """Find matching end tag accounting for nesting, return (chunk_start, chunk_end)."""
        start_tag = f"<{tag_name}"
        end_tag = f"</{tag_name}>"
        tag_stack = []
        ...
        while current_pos < len(content):
            next_start = content.find(start_tag, current_pos + 1)
            next_end = content.find(end_tag, current_pos)
            ...
            if next_start != -1 and next_start < next_end:
                ...
                if not tag_stack:  # This is our matching end tag
                    chunk_end = next_end + len(end_tag)
        ...
        return -1, -1  # Failed to find matching end tag
    ...
    def _extract_xml_chunks(self, content: str) -> List[str]:
        ...
        try:
            while pos < len(content):
                ...
                next_tag_start, current_tag = self._find_next_tool_tag(content, pos)

                if next_tag_start == -1 or not current_tag:
                    break

                # Find the matching end tag
                chunk_start, chunk_end = self._find_matching_end_tag(content, current_tag, next_tag_start)
                ...
                if chunk_start != -1 and chunk_end != -1:
                    ...
                    # If we can't find matching end tag, increment position to avoid infinite loop
    ...
    def _extract_parsing_details(self, xml_chunk: str, tag_name: str, schema) -> Dict[str, Any]:
        """Extract detailed parsing information from XML chunk."""
        parsing_details = {
            "attributes": {},
            "elements": {},
            "text_content": None,
        ...
        for mapping in schema.mappings:
            try:
                if mapping.node_type == "attribute":
                    ...
                    if remaining_chunk and ">" in remaining_chunk:
                    ...
                    if remaining_chunk:
                        content, new_remaining = self._extract_tag_content(remaining_chunk, mapping.path)
                        if content is not None:
                            parsing_details["elements"][mapping.param_name] = content.strip()
                            remaining_chunk = new_remaining
    ...
    def _parse_xml_tool_call(self, xml_chunk: str) -> Optional[Tuple[Dict[str, Any], Dict[str, Any]]]:
        ...
        try:
            ...
            # Extract XML tag name from the text (e.g., "create-file")
            ...
            # Get the actual function name to call (e.g., "create_file")
            function_name = tool_info["method"]
            schema = tool_info["schema"].xml_schema

            # Extract parsing details
            parsing_details = self._extract_parsing_details(xml_chunk, xml_tag_name, schema)

            # Build parameters dictionary from parsing details
            params = {}
            for mapping in schema.mappings:
                if mapping.node_type == "attribute" and mapping.param_name in parsing_details["attributes"]:
                    params[mapping.param_name] = parsing_details["attributes"][mapping.param_name]
                elif mapping.node_type == "element" and mapping.param_name in parsing_details["elements"]:
                    params[mapping.param_name] = parsing_details["elements"][mapping.param_name]
                elif mapping.node_type == "text" and parsing_details["text_content"] is not None:
                    params[mapping.param_name] = parsing_details["text_content"]
                elif mapping.node_type == "content" and parsing_details["root_content"] is not None:
                    params[mapping.param_name] = parsing_details["root_content"]
            ...
            tool_call = {
                "function_name": function_name,  # The actual method to call
                "xml_tag_name": xml_tag_name,  # The original XML tag
    ...
    async def _execute_tool(self, tool_call: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            # Handle string arguments (convert to JSON if possible)
            ...
            logger.error(
                f"Error executing tool {tool_call.get('function_name', 'unknown')}: {str(e)}",
                exc_info=True,
            )
    ...
    async def _execute_tools_sequentially(
        ...
        try:
            ...
            # Create error results for any remaining tools
            error_results = []
            executed_tools = {r[0].get("function_name", "unknown") for r in results}

            for tool in tool_calls:
                if tool.get("function_name", "unknown") not in executed_tools:
    ...
    async def _add_tool_result(
        ...
        This method adds tool results to the conversation history in the appropriate format.
        """
        try:
            # Create metadata
            metadata = {}
            ...
            if "id" in tool_call:
                return await self._add_native_tool_result(thread_id, tool_call, result, metadata)
    ...
    async def _add_native_tool_result(
        self,
        thread_id: str,
        tool_call: Dict[str, Any],
        result: ToolResult,
        ...
        """Add a native tool result (OpenAI format) to the conversation."""
        ...
        if isinstance(result, str):
            ...
            if isinstance(result.output, dict) or isinstance(result.output, list):
                # If output is dict or list, convert to JSON
    ...
    async def _yield_and_save_tool_completed(
        ...
        if context.function_name in ["ask", "complete"]:
            metadata["agent_should_terminate"] = "true"  # Use string "true" instead of boolean
        ...
        return await self._add_status_message(
            ...
            content={
                ...
                "function_name": context.function_name,
                "xml_tag_name": context.xml_tag_name,
                "message": f"Tool {context.xml_tag_name or context.function_name} "
                f"{'completed successfully' if context.result.success else 'failed'}",
                "tool_index": context.tool_index,
    ...
    async def _yield_and_save_tool_error(
        ...
        return await self._add_status_message(
            ...
            content={
                ...
                "function_name": context.function_name,
                "xml_tag_name": context.xml_tag_name,
                "message": f"Error executing tool {context.xml_tag_name or context.function_name}: "
                f"{str(context.error) if context.error else "Unknown error during tool execution"}",
                "tool_index": context.tool_index,
                "tool_call_id": context.tool_call.get("id"),
            },
        )
