...
async def get_user_sandboxes(account_id: str) -> List[Dict[str, Any]]:
    ...
    result = (
        await client.table("projects").select("project_id", "name", "sandbox").eq("account_id", account_id).execute()
    )
    ...
    projects_with_sandboxes = [
        project for project in result.data if project.get("sandbox") and project["sandbox"].get("id")
    ]
