...
async def get_all_customers() -> List[Dict[str, Any]]:
    ...
    result = await client.schema("basejump").from_("billing_customers").select("id", "active").execute()
...
async def update_customer_batch(subscription_status: Dict[str, bool]) -> Dict[str, int]:
    ...
    if active_customers:
        try:
            ...
            await client.schema("basejump").from_("billing_customers").update({"active": True}).in_(
                "id", active_customers
            ).execute()
    ...
    if inactive_customers:
        try:
            ...
            await client.schema("basejump").from_("billing_customers").update({"active": False}).in_(
