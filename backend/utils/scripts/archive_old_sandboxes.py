...
async def get_old_projects(days_threshold: int = 1) -> List[Dict[str, Any]]:
    ...
    while has_more:
        ...
        try:
            result = (
                ...
                .select("project_id", "name", "created_at", "account_id", "sandbox")
    ...
    old_projects_with_sandboxes = [
        ...
        and project.get("created_at") < threshold_date
        and project.get("sandbox")
        and project["sandbox"].get("id")
    ]
...
async def archive_sandbox(project: Dict[str, Any], dry_run: bool) -> bool:
    ...
    try:
        logger.info(
            f"Checking sandbox {sandbox_id} for project '{project_name}' (ID: {project_id}, Created: {created_at})"
        )
